...
class DocxOptimizerEngine:
    ...
    def process(self, input_path, output_path, font_name="Arial", font_size=11,
        ...
        try:
            ...
            # Fix numbering.xml rFonts AND Size to 11pt for all levels
            from lxml import etree as _etree
            ...
            if _num_part is not None:
                ...
                for _lvl in _num_root.iter(f'{{{_WNS_W}}}lvl'):
                    ...
                    # 2. Force Size to 11pt (22 half-points)
                    # Remove old size
                    for _sz in _rPr.findall(f'{{{_WNS_W}}}sz'):
                        _rPr.remove(_sz)
                    ...
                    _sz_new.set(f'{{{_WNS_W}}}val', _sz_val)
                    
                    _szCs_new = _etree.SubElement(_rPr, f'{{{_WNS_W}}}szCs')
                    _szCs_new.set(f'{{{_WNS_W}}}val', _sz_val)
            ...
            for p in paragraphs:
                ...
                is_real_heading = bool(
                    (not is_list_item) and (
                        match_number or has_heading_style or has_numbering or match_bab or
                ...
                if is_real_heading:
                    ...
                    # Format ulang
                    if match_number and not match_bab:
